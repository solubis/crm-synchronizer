...
Ext.util.Observable = Ext.extend(Object, {
	...
	removeManagedListener: function(item, ename, fn, scope) {
		var me = this,
			o, config, managedListeners, managedListener, length, i;
...
Ext.util.Stateful = Ext.extend(Ext.util.Observable, {
	...
	copy: function(newId) {
		...
		this[this.persistanceProperty]), newId || this.internalId);
	}
});
...
Ext.util.GeoLocation = Ext.extend(Ext.util.Observable, {
	...
	setAutoUpdate: function(autoUpdate) {
		...
		try {
			...
			Ext.createDelegate(this.fireUpdate, this), Ext.createDelegate(this.fireError, this), this.parseOptions());
		}
		catch(e) {
	...
	fireError: function(error) {
		this.fireEvent('locationerror', this, error.code == error.TIMEOUT, error.code == error.PERMISSION_DENIED, error.code == error.POSITION_UNAVAILABLE, error.message == undefined ? null : error.message);
	},
...
Ext.util.Offset = Ext.extend(Object, {
	...
	equals: function(offset) {
		if (! (offset instanceof Ext.util.Offset)) throw new Error('offset must be an instance of Ext.util.Offset');
...
function() {
	...
	filterFnPattern = ['var r = [],', 'i = 0,', 'it = arguments[0],', 'l = it.length,', 'c;', 'for (; i < l; i++) {', 'c = it[i].{0};', 'if (c) {', 'r.push(c);', '}', '}', 'return r;'].join(''),
	...
	cq.Query = Ext.extend(Object, {
		...
		is: function(component) {
			var operations = this.operations,
				ln = operations.length,
				i, workingItems = Ext.isArray(component) ? component : [component];
	...
	Ext.apply(this, {
		...
		parse: function(selector) {
			var operations = [],
				ln = matchers.length,
				lastSelector, tokenMatch, matchedChar, modeMatch, selectorMatch, args, i, matcher;
...

	{
		...
		fn: function(doc, div) {
			var prefix = ['webkit', 'Moz', 'o', 'ms', 'khtml'],
				TE = 'TransitionEnd',
				transitionEndName = [
			prefix[0] + TE, 'transitionend', prefix[2] + TE, prefix[3] + TE, prefix[4] + TE],
				ln = prefix.length,
				i = 0,
				out = false;
			div = Ext.get(div);
...
Ext.ModelMgr = new Ext.AbstractManager({
	...
	registerType: function(name, config) {
		...
		var PluginMgr = Ext.PluginMgr,
			...
			association, model, length, i, extendModel, extendModelProto, extendValidations, proxy;
...
Ext.data.TreeStore = Ext.extend(Ext.data.AbstractStore, {
	...
	load: function(options) {
		...
		var node = options.node || this.tree.getRootNode(),
			records, record, reader = this.proxy.reader,
			root;
		...
		return function(options, success, response) {
			if (success === true) {
				var reader = me.getReader(),
					...
					length = records.length,
					mc = new Ext.util.MixedCollection(true, function(r) {
					return r.getId();
				}),
					record, i;
...
Ext.data.Reader = Ext.extend(Object, {
	...
	onMetaChange: function(meta) {
		...
		if (fields) {
			newModel = Ext.regModel("JsonReader-Model" + Ext.id(), {
				fields: fields
			});
...
Ext.Application = Ext.extend(Ext.util.Observable, {
	...
	constructor: function(config) {
		...
		if (name) {
			...
			Ext.ns(
			name, name + ".views", name + ".stores", name + ".models", name + ".controllers");
		}
	...
	onBeforeLaunch: function() {
		...
		if (useHistory) {
			...
			if (this.launch.call(this.scope || this, profile) !== false) {
				Ext.redirect(token || this.defaultUrl || {
...
Ext.applyIf(Ext.Element, {
	...
	cssTransform: function(el, transforms) {
		...
		if (Ext.supports.CSS3DTransform) {
			el.style.webkitTransform = 'matrix3d(' + m.m11 + ', ' + m.m12 + ', ' + m.m13 + ', ' + m.m14 + ', ' + m.m21 + ', ' + m.m22 + ', ' + m.m23 + ', ' + m.m24 + ', ' + m.m31 + ', ' + m.m32 + ', ' + m.m33 + ', ' + m.m34 + ', ' + m.m41 + ', ' + m.m42 + ', ' + m.m43 + ', ' + m.m44 + ')';
		} else {
...
(function() {
	...
	El.addMethods({
		...
		getStyle: function(prop) {
			var dom = this.dom,
				result, display, cs, platform = Ext.is,
				style = dom.style;
			...
			if (result && !platform.correctRightMargin && this.marginRightRe.test(prop) && style.position != 'absolute' && result != '0px') {
...
Ext.Element.addMethods({
	...
	insertSibling: function(el, where, returnDom) {
		var me = this,
			rt, isAfter = (where || 'before').toLowerCase() == 'after',
			insertEl;
	...
	wrap: function(config, returnDom) {
		var newEl = Ext.DomHelper.insertBefore(this.dom, config || {
			tag: "div"
		},
		!returnDom);
...
Ext.Element.addMethods({
	...
	getAlignToXY: function(el, position, offsets) {
		...
		var me = this,
			...
			p1 = "",
			p2 = "",
			a1, a2, x, y, swapX, swapY, p1x, p1y, p2x, p2y, width, height, region, constrain;
...
Ext.Viewport = new(Ext.extend(Ext.util.Observable, {
	...
	onResize: function() {
		if (this.orientation != this.getOrientation()) {
			...
			if (!Ext.is.iOS && !Ext.is.Desktop) {
				if ((size.width == this.lastSize.width && size.height > this.lastSize.height) || (size.height == this.lastSize.height && size.width > this.lastSize.width)) {
	...
	getSize: function() {
		...
		if (!Ext.is.Desktop) {
			size.height = (this.orientation == this.initialOrientation) ? Math.max(this.initialHeight, size.height) : size.height
		}
...
Ext.util.TapRepeater = Ext.extend(Ext.util.Observable, {
	...
	tap: function(e) {
		this.fireEvent("tap", this, e);
		this.timer = Ext.defer(this.tap, this.accelerate ? this.easeOutExpo(Ext.util.Date.getElapsed(this.tapStartTime), 400, -390, 12000) : this.interval, this, [e]);
	},
...
Ext.util.Draggable = Ext.extend(Ext.util.Observable, {
	...
	setOffset: function(offset, animate) {
		...
		if (!this.offset.equals(offset)) {
			if (animate) {
				...
				this.initialRegion.top + offset.y, this.initialRegion.right + offset.x, this.initialRegion.bottom + offset.y, this.initialRegion.left + offset.x);
	...
	getNewOffsetFromTouchPoint: function(touchPoint) {
		...
		if (this.horizontal) newOffset.x = this.startOffset.x + xDelta;

		if (this.vertical) newOffset.y = this.startOffset.y + yDelta;
...
(function() {
	...
	Ext.util.ScrollView = Ext.extend(Ext.util.Observable, {
		...
		constructor: function(el, config) {
			...
			indicators.forEach(function(i) {
				this.indicators[i] = new Ext.util.Scroller.Indicator(this.scroller.container, Ext.apply({},
		...
		onScroll: function(scroller) {
			if (scroller.offsetBoundary == null || (!this.indicators.vertical && !this.indicators.horizontal)) return;

			var sizeAxis, offsetAxis, offsetMark, boundary = scroller.offsetBoundary,
			...
			Ext.iterate(this.indicators, function(axis, indicator) {
				...
				if (this.boundarySize[sizeAxis] != 0) {
					this.indicatorOffsets[axis] = (((boundary[offsetMark] - this.restrictedOffset[offsetAxis]) / this.boundarySize[sizeAxis]) * (this.containerSize[sizeAxis] - this.indicatorSizes[axis]));
	...
	Ext.util.Scroller = Ext.extend(Ext.util.Draggable, {
		...
		onStart: function(e) {
			if (Ext.util.Scroller.superclass.onStart.apply(this, arguments) !== true) return;
		...
		onDrag: function(e) {
			if (Ext.util.Scroller.superclass.onDrag.apply(this, arguments) !== true) return;
		...
		onDragEnd: function(e) {
			if (Ext.util.Scroller.superclass.onDragEnd.apply(this, arguments) !== true) return;
		...
		startMomentumAnimation: function(e) {
			...
			if ((!this.momentum || !(duration <= this.startMomentumResetTime)) && !this.offsetBoundary.isOutOfBound(this.offset)) {
				return false;
			}

			var minVelocity = this.minVelocityForAnimation,
				currentVelocity, currentOffset = this.offset.copy(),
				restrictedOffset, acceleration = (duration / this.acceleration);
		...
		handleMomentumAnimationFrame: function() {
			...
			var currentTime = Date.now(),
				...
				currentVelocity, restrictedOffset, outOfBoundDistance;
...
(function() {
	...
	Ext.apply(Date, {
		...
		createParser: function() {
			var code = ["var dt, y, m, d, h, i, s, ms, o, z, zz, u, v,", "def = Date.defaults,", "results = String(input).match(Date.parseRegexes[{0}]);",
			...
			"y = Ext.num(y, Ext.num(def.y, dt.getFullYear()));", "m = Ext.num(m, Ext.num(def.m - 1, dt.getMonth()));", "d = Ext.num(d, Ext.num(def.d, dt.getDate()));",

			"h  = Ext.num(h, Ext.num(def.h, dt.getHours()));", "i  = Ext.num(i, Ext.num(def.i, dt.getMinutes()));", "s  = Ext.num(s, Ext.num(def.s, dt.getSeconds()));", "ms = Ext.num(ms, Ext.num(def.ms, dt.getMilliseconds()));",

			"if(z >= 0 && y >= 0){",

			"v = new Date(y, 0, 1, h, i, s, ms);",

			"v = !strict? v : (strict === true && (z <= 364 || (v.isLeapYear() && z <= 365))? v.add(Date.DAY, z) : null);", "}else if(strict === true && !Date.isValid(y, m + 1, d, h, i, s, ms)){",
			...
			"v = v.add(Date.SECOND, -v.getTimezoneOffset() * 60 - zz);", "}else if(o){",
		...
		parseCodes: {
			...
			c: function() {
...

				{
					...
					"}else if (results[8].indexOf(':') > -1){", $f("P", 8).c,
...
Ext.apply(Date.prototype, {
	...
	getGMTOffset: function(colon) {
		return (this.getTimezoneOffset() > 0 ? "-" : "+") + Ext.util.Format.leftPad(Math.floor(Math.abs(this.getTimezoneOffset()) / 60), 2, "0") + (colon ? ":" : "") + Ext.util.Format.leftPad(Math.abs(this.getTimezoneOffset() % 60), 2, "0");
	},
	...
	request: function(o) {
		...
		if (me.fireEvent('beforerequest', me, o) !== false) {
			var params = o.params,
				...
				request, data, headers, method, key, xhr;
...
Ext.util.EventSimulator = Ext.extend(Object, {
	...
	getEventTypeByName: function(name) {
		...
		Ext.iterate(this.supportedEvents, function(type, events) {
			if (events.indexOf(name) != -1) ret = type;
		});
	...
	createTouchEvent: function(type, data) {
		...
		if (touchEvent.initTouchEvent.length == 9) {
			touchEvent.initTouchEvent(data.touches, data.targetTouches, data.changedTouches, type, data.view, data.screenX, data.screenY, data.clientX, data.clientY);

		} else {
			touchEvent.initTouchEvent(type, data.bubbles, data.cancelable, data.view, data.detail, data.screenX, data.screenY, data.pageX, data.pageY, data.ctrlKey, data.altKey, data.shiftKey, data.metaKey, data.touches, data.targetTouches, data.changedTouches, data.scale, data.rotation);
		}
	...
	createTouch: function(target, options, serializable) {
		...
		document.defaultView, target, +options.identifier || 0, +options.pageX || 0, +options.pageY || 0, +options.screenX || 0, +options.screenY || 0);
	},
	...
	createMouseEvent: function(type, data) {
		...
		type, data.bubbles, data.cancelable, data.view, data.detail, data.screenX, data.screenY, data.clientX, data.clientY, data.ctrlKey, data.altKey, data.shiftKey, data.metaKey, data.button, data.relatedTarget);
...
Ext.util.EventRecorder = Ext.extend(Ext.util.Observable, {
	...
	getEventSet: function(name) {
		if (typeof name != 'string') {
			if (this.currentEventSetName == null) throw new Error('No EventSet is currently used');
	...
	record: function(name, event) {
		if (typeof name != 'string') {
			...
			if (this.currentEventSetName == null) return;
	...
	replay: function(name, startIndex) {
		var simulator = this.getEventSimulator(),
			...
			numEvents, delay = 0,
			index = 0,
			event, point, target, reserveTargetEvents = ['touchmove', 'touchend', 'mousemove', 'mouseup'],
			me = this;
	...
	setEventSimulator: function(eventSimulator) {
		if (! (eventSimulator instanceof Ext.util.EventSimulator)) throw new Error('eventSimulator must be an instance of Ext.util.EventSimulator');
...
Ext.gesture.Tap = Ext.extend(Ext.gesture.Gesture, {
	...
	fireTapEvent: function(e, info) {
		...
		if (!target.disabled && this.fireClickEvent) {
			...
			clickEvent.initMouseEvent('click', e.bubbles, e.cancelable, document.defaultView, e.detail, e.screenX, e.screenY, e.clientX, e.clientY, e.ctrlKey, e.altKey, e.shiftKey, e.metaKey, e.metaKey, e.button, e.relatedTarget);
	...
	isCancel: function(touch) {
		var me = this;
		return (
		Math.abs(touch.pageX - me.startX) >= me.cancelThreshold || Math.abs(touch.pageY - me.startY) >= me.cancelThreshold);
	}
});
...
Ext.gesture.Drag = Ext.extend(Ext.gesture.Touch, {
	...
	isDragging: function(info) {
		return ((this.horizontal && info.absDeltaX >= this.dragThreshold) || (this.vertical && info.absDeltaY >= this.dragThreshold));
	},
...
Ext.gesture.Pinch = Ext.extend(Ext.gesture.Gesture, {
	...
	getDistance: function() {
		var me = this;
		return Math.sqrt(
		Math.pow(Math.abs(me.firstTouch.pageX - me.secondTouch.pageX), 2) + Math.pow(Math.abs(me.firstTouch.pageY - me.secondTouch.pageY), 2));
	}
});
...
Ext.lib.Component = Ext.extend(Ext.util.Observable, {
	...
	constructor: function(config) {
		...
		'beforestaterestore', 'staterestore', 'beforestatesave', 'statesave');
...
Ext.Component = Ext.extend(Ext.lib.Component, {
	...
	alignTo: function(alignTo, allowSides, offset) {
		...
		var alignBox = alignTo.getPageBox(),
			...
			position, index = 2,
			positionMap = ['tl-bl', 't-b', 'tr-br', 'l-r', 'l-r', 'r-l', 'bl-tl', 'b-t', 'br-tr'],
			anchorEl = this.anchorEl,
			offsets = [0, offset],
			targetBox, cls, onSides = false,
			arrowOffsets = [0, 0],
...
Ext.lib.Container = Ext.extend(Ext.Component, {
	...
	removeAll: function(autoDestroy) {
		var item, removeItems = this.items.items.slice(),
			items = [],
...
Ext.Container = Ext.extend(Ext.lib.Container, {
	...
	handleFieldEventListener: function(isAdding, item) {
		if (!this.fieldEventWrap) this.fieldEventWrap = {};

		if (['textfield', 'passwordfield', 'emailfield', 'textareafield', 'searchfield', 'urlfield', 'numberfield', 'spinnerfield'].indexOf(item.xtype) !== -1) {
			if (isAdding) {
				this.fieldEventWrap[item.id] = {
					beforefocus: function(e) {
						this.onFieldBeforeFocus(item, e);
					},
					focus: function(e) {
						this.onFieldFocus(item, e);
					},
					blur: function(e) {
						this.onFieldBlur(item, e);
					},
					keyup: function(e) {
						this.onFieldKeyUp(item, e);
					},
					scope: this
				};
		...
		if (!Ext.is.iOS && !Ext.is.Desktop) {
			...
			if (this.focusingField == field || !Ext.is.iOS) {
				...
				if (this.lastFocusedField) {
					...
					if (!containerRegion.contains(fieldRegion) && (offsetY != 0 || (offsetY == 0 && scroller.offset.y != 0))) {
						selfHandling = true;
					}
...
Ext.AbstractStoreSelectionModel = Ext.extend(Ext.util.Observable, {
	...
	selectRange: function(startRecord, endRecord, keepExisting, dir) {
		var i, startRow = this.store.indexOf(startRecord),
...
Ext.DataView = Ext.extend(Ext.Component, {
	...
	prepareAssociatedData: function(record, ids) {
		...
		var associations = record.associations.items,
			...
			associationData = {},
			associatedStore, associatedName, associatedRecords, associatedRecord, associatedRecordCount, association, internalId, i, j;
...
Ext.List = Ext.extend(Ext.DataView, {
	...
	onScroll: function(scroller, pos, options) {
		...
		if (pos.y <= 0) {
			...
			return;
		} else if ((this.closest.next && pos.y > this.closest.next.offset) || (pos.y < this.closest.current.offset)) {
			this.closest = this.getClosestGroups(pos);
			this.setActiveGroup(this.closest.current);
		}
	...
	onIndex: function(record, target, index) {
		...
		if (closest) {
			this.scroller.scrollTo({
				x: 0,
				y: closest.getOffsetsTo(this.scrollEl)[1]
			},
			false, null, true);
		}
	},
...
Ext.IndexBar = Ext.extend(Ext.DataView, {
	...
	onTouchMove: function(e) {
		...
		var point = Ext.util.Point.fromEvent(e),
			target, record, pageBox = this.pageBox;
...
Ext.Carousel = Ext.extend(Ext.Panel, {
			...
			if (this.isCardInRange(card)) {
				...
				if (animate) {
					if (card === this.layout.activeItem) {
						el.on('webkitTransitionEnd', this.onTransitionEnd, this, {
							single: true
						});
					}
				...
				if (this.horizontal) {
					Ext.Element.cssTransform(el, {
						translate: [cardOffset, 0]
					});
				} else {
					Ext.Element.cssTransform(el, {
						translate: [0, cardOffset]
					});
				}
			} else if (!card.hidden) {
	...
	getCardOffset: function(card) {
		...
		return this.horizontal ? (cardOffset * currentSize.width) + currentScroll.x : (cardOffset * currentSize.height) + currentScroll.y;
	},
...
Ext.NestedList = Ext.extend(Ext.Panel, {
	...
	addNextCard: function(recordNode, swapTo) {
		var nextList, parentNode = recordNode ? recordNode.parentNode : null,
		card;
	...
	setActivePath: function(path) {
		...
		var gotoRoot = path.substr(0, 1) === "/",
			...
			ds = this.store,
			tree = ds.tree,
			node, card, lastCard, pathArr, pathLn;
	...
	onClick: function(button) {
		if (button) {
			...
			if (typeof config.fn == 'function') {
				config.fn.call(
				config.scope || null, button.itemId || button.text, config.input ? config.input.dom.value : null, config);
			}
		...
		if (this.inputsEl && (config.multiLine || config.prompt)) {
			...
			for (attrName in attribs) {
				if (attribs.hasOwnProperty(attrName)) {
					...
					attrName.toLowerCase(), /^auto/i.test(attrName) ? (assert.test(attrib + '') ? 'on' : 'off') : attrib);
				}
			}
...
Ext.form.Radio = Ext.extend(Ext.form.Checkbox, {
	...
	getGroupValue: function() {
		var field, fields = this.getSameGroupFields();
	...
	getTarget: function() {
		...
		if (!innerCt) {
			if (owner.scrollEl) {
				...
				innerCt = owner.getTargetEl().createChild({
					cls: this.innerCls
				});
			}
